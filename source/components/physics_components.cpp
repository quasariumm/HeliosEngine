...
void PhysicsComponent::Initialize( const entt::entity owner )
{
	if (initialized)
	{
	...
	m_transform = &GetComponent<Transform>(owner);
	m_transform->SubscribeOnModify<&PhysicsComponent::OnTransformModified>(this);
...
void PhysicsComponent::OnTransformModified() const
{
	...
	Systems::GetPhysics()->BodyInterface().SetPositionAndRotation(m_bodyId,
	                                                              {pos.x, pos.y, pos.z},
...
void PhysicsComponent::Inspector()
{
	...
	if (const ImVec2 area = {ImGui::GetContentRegionAvail().x, ImGui::GetFontSize() * 1.5f};
		ImGui::BeginChild("Movement selector", area))
	{
		const ImVec2 size = {area.x / 3.0f, area.y};
		...
		if (ImGui::Selectable("Static", motionType == EMotionType::Static, ImGuiSelectableFlags_None, size))
		{
			...
			Systems::GetPhysics()->BodyInterface().SetMotionType(m_bodyId, motionType, EActivation::Activate);
		}
		ImGui::SameLine();
...
void PhysicsSphere::Initialize( const entt::entity owner )
{
	...
	m_bodyId = Systems::GetPhysics()->BodyInterface().CreateAndAddBody(sphereSettings, EActivation::Activate);

	OnTransformModified();
}


void PhysicsCube::Initialize( const entt::entity owner )
{
	PhysicsComponent::Initialize(owner);
	...
	m_bodyId = Systems::GetPhysics()->BodyInterface().CreateAndAddBody(cubeSettings, EActivation::Activate);
