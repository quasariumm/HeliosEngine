...
void Camera::HandleInput( Window& window, const float deltaTime )
{
	...
	if (window.GetKey(Key::LEFT_SHIFT))
		speed *= 5.0f;
	if (window.GetKey(Key::LEFT_CONTROL))
		speed *= 0.2f;

	UpdateCameraVectors();
	bool changed = false;
	if (window.GetKey(Key::A))
		m_camPos -= m_camRight * speed, changed = true;
	if (window.GetKey(Key::D))
		m_camPos += m_camRight * speed, changed = true;
	if (window.GetKey(Key::W))
		m_camPos += m_camFront * speed, changed = true;
	if (window.GetKey(Key::S))
		m_camPos -= m_camFront * speed, changed = true;
	if (window.GetKey(Key::R))
		m_camPos += m_camUp * speed, changed = true;
	if (window.GetKey(Key::F))
		m_camPos -= m_camUp * speed, changed = true;
	if (!changed)
		return;
	UpdateCameraVectors();
