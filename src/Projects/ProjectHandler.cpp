...
namespace Engine {
    ...
    bool ProjectHandler::CreateProject(std::filesystem::path &projectPath, const ProjectData &data) {
        ...
        // Adding a CMake file
        std::filesystem::path projectCmakePath = projectPath;
        projectCmakePath.append("CMakeLists.txt");
        if (!DefaultFile(DefaultFileType::CMAKELISTS, projectCmakePath)) {
        ...
        headerPath.append(data.projectName + STR_TO_WSTR(".h"));
        if (!DefaultFile(DefaultFileType::MAINHEADER, headerPath)) {
            DebugLog(LogSeverity::SEVERE, L"Could not create header file");
            return false;
        }

        std::filesystem::path sourcePath = sourceFolder;
        sourcePath.append(data.projectName + STR_TO_WSTR(".cpp"));
        if (!DefaultFile(DefaultFileType::MAINSOURCE, sourcePath)) {
    ...
    bool ProjectHandler::RecompileProject() {
        ...
        int buildStatus = std::system(
                ("cmake --build " + ProjectFolder().generic_string() + "\\cmake-build-debug").c_str());
        if (buildStatus != 0) {
